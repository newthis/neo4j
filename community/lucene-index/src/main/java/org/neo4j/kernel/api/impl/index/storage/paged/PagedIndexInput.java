...
public class PagedIndexInput extends IndexInput implements RandomAccessInput
{
    ...
    private void moveCursor( long pageId, int offsetInPage ) throws IOException
    {
        ...
        {
            throw newEOFException( String.format( "Page cache EOF for { pageId: %d, offsetInPage: %d }",
                    pageId, offsetInPage ) );
        }

        if( isEOF( pageId, offsetInPage ) )
        {
            throw newEOFException( String.format( "moveCursor EOF check failed for { pageId: %d, offsetInPage: %d }",
                    pageId, offsetInPage ) );
        }
    }
